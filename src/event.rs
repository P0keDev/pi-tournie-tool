...
use crate::gpio::{Button, GpioHandler};
...
impl EventHandler {
    ...
    pub fn new() -> Self {
        ...
        let gpio = GpioHandler::new(sender.clone());
        thread::spawn(|| gpio.run());
    ...
    pub fn send_gpio(&mut self, button: Button) {
        let _ = self.sender.send(Event::Gpio(button));
    }
}
